<script lang="ts">
  let { title, detail, extraClass = '', href } = $props();
  const imgSrc =
  ...
  let cardClass =
    'card card-hover block w-full divide-y overflow-hidden border-[1px] border-surface-200-800 divide-surface-200-800 preset-filled-surface-100-900';
  if (extraClass !== '') {
    cardClass += extraClass;
  }
</script>
